...
import { createMealFoodRepository, deleteMealFoodRepository, getMealFoodByIdRepository, getMealFoodsRepository } from "../repositories/meals_foods.repository.js";
import type { CreateMealFoodDTO } from "../types/meal-foods.type.js";
...
export async function deleteMealFoodService(mealId: number, mealFoodId: number, userId: number) {
    ...
    if (!mealFoodExists) {
        throw new AppError(`Food with ID ${mealFoodId} not found`, 404)
    }
